...
export function generateTriangularGadgetGraph(opts = {}) {
    const G = {
        const l = { source: String(source), target: String(target), weight: nextLabel() };
        // keep backward-compatible `label` field for other callers
        l.label = String(l.weight);
        links: [],
    };
    ...
    const TOP_GADGETS = opts.TOP_GADGETS ?? 6;  // number of triangular gadgets on the top row (a1..a6)
    const BOTTOM_GADGETS = opts.BOTTOM_GADGETS ?? 3; // number on bottom row (a7..a9)
    ...
    function addLink(source, target) {
        const w = nextLabel();
        const l = { source: String(source), target: String(target), weight: w };
        G.links.push(l);
        return l;
    }
    ...
    // Connect root v to a1
    addLink(vId, topA[0]);

    // Top gadget edges and spine links (a1-c1-a2-c2-...)
    for (let i = 0; i < TOP_GADGETS; i++) {
        addLink(topA[i], topC[i]); // base edge along spine
        addLink(topA[i], topB[i]); // left leg
        addLink(topB[i], topC[i]); // right leg
        if (i < TOP_GADGETS - 1) addLink(topC[i], topA[i+1]); // spine connection to next a
    }

    // Antennae from each b_i (top row) with leaf nodes (up-right, leaves to the right)
    for (let i = 0; i < TOP_GADGETS; i++) {
        const bi = topB[i];
        let prev = bi;
    ...
    // Create the curved turn from c_TOP_GADGETS to the start of bottom gadgets
    ...
    // Bottom row gadgets: place BOTTOM_GADGETS mirrored going right-to-left and numbered sequentially
    const bottomA = [];
    const bottomB = [];
    const bottomC = [];
    const bottomStartX = G.nodes.find(n => n.id === turnNodes[turnNodes.length - 1]).x + TURN_RIGHT_OFFSET;
    for (let i = 0; i < BOTTOM_GADGETS; i++) {
        // overall index for numbering: TOP_GADGETS + i + 1
        const idx = TOP_GADGETS + i + 1;
        // index i=0 is the rightmost triangle on the bottom row (a7)
    ...
    // connect turn end to first bottom a (the rightmost bottom a = a{TOP_GADGETS+1})
    ...
    // Antennae from bottom b nodes (down-right, leaves to the left)
    for (let i = 0; i < BOTTOM_GADGETS; i++) {
        const idx = TOP_GADGETS + i + 1;
        let prev = bottomB[i];
        let baseX = (G.nodes.find(n => n.id === prev).x);
        ...
        for (let d = 1; d <= ANTENNA_DEPTH; d++) {
            ...
            addLink(prev, wId);
            const leafId = `leaf_${idx}_${d}`;
            // leaf offset to the left for bottom row
            addNode(leafId, wX - LEAF_OFFSET, wY, leafId);
            addLink(wId, leafId);
            prev = wId;
        }
